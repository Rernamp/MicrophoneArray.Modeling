...
def _(mo):
    ...
    number_elements_ui = mo.ui.slider(value=4, start=2, stop=32, step=1, label="Number elements", include_input=True, show_value=True)

    angle_of_arrival_ui = mo.ui.slider(value=0, start=-90, stop=90, step=0.1, label="Angle of arrival (deg)", include_input=True, show_value=True)
    mic_distance_ui = mo.ui.slider(value=2e-2, start=1e-2, stop=1, step=1e-3, label="Distance between adjacent elements (m)", include_input=True, show_value=True)
    sample_rate_ui = mo.ui.slider(value=48e3, start=8e3, stop=192e3, step=1, label="Sample rate (Hz)", include_input=True, show_value=True)
    return (
        angle_of_arrival_ui,
        mic_distance_ui,
        number_elements_ui,
